    ...
    while True:
        ...
        while True:                                      #mole fractions
            ...
            if abs(sum(y))== 1:
                ...
                print(f"Error: Sum of mole fractions = {sum(y):.6f}")
        ...
        while True:
            try:
                T_tol_percent = float(input("\nEnter error tolerance (%) for iteration of T (e.g., 0.01 for 0.01%): "))
                if T_tol_percent > 0:
                    break
                else:
        ...
        gamma_tol = float(input("Enter gamma convergence tolerance (%): "))
        ...
        Results = dew_point_iteration(T0, P, y, A, B, C, T_unit_Ant, V, a, R, k_index, P_unit_Ant, T_tol, nonideal_liq, nonideal_gas, Bik, gamma_tol)

        print("\nChoose next action:")
        print("1 - Change pressure P, vapor composition y, fixed component, gamma convergence tolerance, T convergence tolerance")
        ...
        if nonideal_liq:
            ...
            while True:
                ...
                if inner_iter >= max_inner_iter:
                    print("Warning: gamma did not converge")
                    gamma = gamma_new
                    break
...
######
        ...
        print("i   Pi_sat (kPa)     gamma_i        alpha_ik        Xi")
        print("-" * 60)

        for i in range(len(y)):
            print(f"{i+1:<3} {Pi_sat[i]:<14.8f} {gamma[i]:<14.8f} {alpha[i]:<14.8f} {x[i]:<14.8f}")
    ...
    print("Liquid phase mole fractions xi:")
